...
*   **Shadowing Conflicts:** When using FSet with `cl`, always use `:shadowing-import-from #:fset` for symbols like `map`, `set`, `reduce`, `union`, and `lookup`. Failure to do this results in `NAME-CONFLICT` errors during load.
...
*   **Rounding Behavior:** In Common Lisp, `(round 2.5)` returns `2`, and `(round 3.5)` returns `4`. It rounds to the **nearest even number**. This is mathematically sound but disastrous for grid-based game logic.
    *   **Standard Game Rounding:** Use `(floor (+ x 0.5))` for consistent "Round Half Up" behavior.
...
To ensure a clean environment and run all tests:
```bash
find . -name "*.fasl" -delete && sbcl --non-interactive --load foldback.asd --eval "(asdf:load-system :foldback :force t)" --load tests/unit-tests.lisp
```
