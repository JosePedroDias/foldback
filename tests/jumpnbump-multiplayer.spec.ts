...
async function testMultiplayer(browser, game, protocol) {
  ...
  // Both should see at least 2 players
...
test('Jump and Bump: Two players can connect via WebRTC', async ({ browser }) => {
  await testMultiplayer(browser, 'jumpnbump', 'webrtc');
});

test('Jump and Bump: Two players can connect via WebSockets', async ({ browser }) => {
  await testMultiplayer(browser, 'jumpnbump', 'websockets');
});
