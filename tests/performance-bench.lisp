...
(defun run-variant (name width height players)
  (format t "~%=== VARIANT: ~A (~Ax~A, ~A Players) ===~%" name width height players)
                           ...
                           (let ((start-port (+ 15000 (random 5000))))
                             (loop for p from start-port to (+ start-port 10)
                                   do (handler-case
                                          (progn
                                            (setf actual-port p)
                                            (return (start-server :port p :max-ticks 600)))
...
(run-variant "V1: Small" 13 11 4)
(run-variant "V2: Large" 31 31 20)
