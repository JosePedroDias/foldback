...
(defun test-rollback-idempotency ()
  ...
  (let* (;; Initial State with one player at (0, 0)
         (cs (map (:level (make-level 10 10)) (:bombs (map)) (:explosions (map)) (:bots (map))))
         (s0 (with (initial-state :custom-state cs) :players (map (:p1 (make-player :x 0 :y 0)))))
    ...
    ;; Trigger rollback from tick 1 (re-simulating i2 on s1)
